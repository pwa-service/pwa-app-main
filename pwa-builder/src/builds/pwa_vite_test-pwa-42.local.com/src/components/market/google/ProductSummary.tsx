import type { IProductSummaryData } from "../../../types/market";
import { classNames } from "../../../utils/classNames";
import { Fragment } from "react";

interface IProductSummaryProps {
  productSummary: IProductSummaryData[];
}
        ...
        return (
          <Fragment key={id}>
            <div className="h-12 flex flex-col items-center justify-between shrink-0 px-4">
              <div className="flex-1 flex items-center gap-[1px] justify-center">
                {value && (
                  <p className="font-medium text-[#0D0D0D] sm:mt-0 xl:mt-[3px]">
                    {value}
                  </p>
                )}
              ...
              <p className="font-normal text-[#5C5C5C] text-sm shrink-0">
                {label}
              </p>
            </div>

            {!isLast && (
              <div className="shrink-0 w-[1px] h-8 mx-3 bg-black/50" />
            )}
          </Fragment>
        );
      })}
