...
export class MemberHttpController {
    ...
    async getMyStats(@Req() req: any) {
        return this.client.getMyStats(req.user.id);
    }

    @Post('campaign')
    @ApiOperation({ summary: 'Create a new member directly in a campaign' })
    async createCampaignMember(@Body() dto: CreateCampaignMemberDto) {
        return this.client.createCampaignMember(dto);
    }

    @Post('team/lead')
    @ApiOperation({ summary: 'Create a Team Lead (requires Campaign Owner rights)' })
    async createTeamLead(@Body() dto: CreateMemberDto) {
        return this.client.createTeamLead(dto);
    }

    @Post('team/member')
    @ApiOperation({ summary: 'Create a standard Team Member' })
    async createTeamMember(@Body() dto: CreateMemberDto) {
        return this.client.createTeamMember(dto);
    }
}