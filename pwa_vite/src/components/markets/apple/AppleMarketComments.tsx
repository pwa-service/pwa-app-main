  ...
  return (
    <div>
      {comments.map((comment) => (
        ...
        >
          <h3 className="font-medium sm:text-[14px] text-[#202124]">{comment.fullName}</h3>
          ...
          <div className="flex items-center">
            ...
            <p className="ml-[10px] font-normal sm:text-[12px] text-[#4C4D55]">{comment.date}</p>
          </div>

          <p className="font-normal sm:text-[12px] text-[#4C4D55] leading-[105%]">{comment.text}</p>
        </div>
      ))}
    </div>
  );
